...
def ParseInt(value: str | None, base: int = 10) -> Validation[int, str]:
    return (
        ...
        .Bind(lambda text: Validation.Try(
            lambda: int(text, base),
            lambda exception: [f"ParseInt: cannot parse {value!r}: {exception}"],
        ))
    )
...
def ParseFloat(value: str | None) -> Validation[float, str]:
    def _Parse(text: str) -> Validation[float, str]:
        return (
            Validation.Try(
                lambda: float(text),
                lambda exception: [f"ParseFloat: cannot parse {value!r}: {exception}"],
            )
            .Bind(lambda floatValue: (
                Validation.Success(floatValue)
                if math.isfinite(floatValue)
                else Validation.Fail([f"ParseFloat: {value!r} is not finite"])
            ))
        )
...
def ParseDecimal(value: str | None) -> Validation[Decimal, str]:
    def _Parse(text: str) -> Validation[Decimal, str]:
        return (
            Validation.Try(
                lambda: Decimal(text),
                lambda exception: [f"ParseDecimal: cannot parse {value!r}: {exception}"],
            )
            .Bind(lambda decimalValue: (
                Validation.Success(decimalValue)
                if decimalValue.is_finite()
                else Validation.Fail([f"ParseDecimal: {value!r} is not finite"])
            ))
        )
...
def ParseDate(value: str | None, format: str = "%Y-%m-%d") -> Validation[date, str]:
    return (
        ...
        .Bind(lambda text: Validation.Try(
            lambda: datetime.strptime(text, format).date(),
            lambda exception: [
                f"ParseDate: cannot parse {value!r} with format {format!r}: {exception}"
            ],
        ))
    )

def ParseDatetime(
    value: str | None, format: str = "%Y-%m-%dT%H:%M:%S"
) -> Validation[datetime, str]:
    return (
        _ValidateString(value, "ParseDatetime")
        .Bind(lambda text: Validation.Try(
            lambda: datetime.strptime(text, format),
            lambda exception: [
                f"ParseDatetime: cannot parse {value!r} with format {format!r}: {exception}"
            ],
        ))
    )
...
def ParseTime(value: str | None, format: str = "%H:%M:%S") -> Validation[time, str]:
    return (
        ...
        .Bind(lambda text: Validation.Try(
            lambda: datetime.strptime(text, format).time(),
            lambda exception: [
                f"ParseTime: cannot parse {value!r} with format {format!r}: {exception}"
            ],
        ))
    )
...
def ParseUuid(value: str | None) -> Validation[UUID, str]:
    return (
        ...
        .Bind(lambda text: Validation.Try(
            lambda: UUID(text),
            lambda exception: [f"ParseUuid: cannot parse {value!r}: {exception}"],
        ))
    )

def ParseEnum(
    value: str | None, enumType: type[Enum], caseSensitive: bool = False
) -> Validation[Enum, str]:
...
def ParseRegex(value: str | None, pattern: str, group: int | str = 0) -> Validation[str, str]:
    def _Match(text: str) -> Validation[str, str]:
        ...
        return Validation.Try(
            lambda: matchResult.group(group),
            lambda exception: [f"ParseRegex: group {group!r} not found: {exception}"],
        )
