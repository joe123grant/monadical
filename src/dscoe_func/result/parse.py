...
def ParseInt(value: str | None, base: int = 10) -> Result[int]:
    return (
        ...
        .Bind(lambda text: Result.Try(
            lambda: int(text, base),
            lambda exception: ValueError(f"ParseInt: cannot parse {value!r}: {exception}"),
        ))
    )
...
def ParseFloat(value: str | None) -> Result[float]:
    def _Parse(text: str) -> Result[float]:
        return (
            Result.Try(
                lambda: float(text),
                lambda exception: ValueError(f"ParseFloat: cannot parse {value!r}: {exception}"),
            )
            .Bind(lambda floatValue: (
                Result.Success(floatValue)
                if math.isfinite(floatValue)
                else Result.Fail(ValueError(f"ParseFloat: {value!r} is not finite"))
            ))
        )
        ...
        return (
            Result.Try(
                ...
                lambda exception: ValueError(
                    f"ParseDecimal: cannot parse {value!r}: {exception}"
                ),
            )
            .Bind(lambda decimalValue: (
                Result.Success(decimalValue)
                if decimalValue.is_finite()
                else Result.Fail(ValueError(f"ParseDecimal: {value!r} is not finite"))
            ))
        )
...
def ParseUuid(value: str | None) -> Result[UUID]:
    return (
        ...
        .Bind(lambda text: Result.Try(
            lambda: UUID(text),
            lambda exception: ValueError(f"ParseUuid: cannot parse {value!r}: {exception}"),
        ))
    )
...
def ParseEnum(value: str | None, enumType: type[Enum], caseSensitive: bool = False) -> Result[Enum]:
    def _Lookup(text: str) -> Result[Enum]:
        ...
        return Result.Fail(
            ValueError(f"ParseEnum: {value!r} is not a member of {enumType.__name__}")
        )
...
def ParseRegex(value: str | None, pattern: str, group: int | str = 0) -> Result[str]:
    def _Match(text: str) -> Result[str]:
        ...
        if matchResult is None:
            return Result.Fail(
                ValueError(f"ParseRegex: pattern {pattern!r} did not match {value!r}")
            )
        return Result.Try(
            lambda: matchResult.group(group),
            lambda exception: ValueError(
                f"ParseRegex: group {group!r} not found: {exception}"
            ),
        )
