...
def ParseInt(value: str | None, base: int = 10) -> Validation[int, str]:
    return (
        ...
        .Then(lambda s: Validation.Try(lambda: int(s, base), lambda e: [f"ParseInt: cannot parse {value!r}: {e}"]))
    )
...
def ParseFloat(value: str | None) -> Validation[float, str]:
    def _parse(s: str) -> Validation[float, str]:
        return (
            ...
            .Then(lambda f: Validation.Success(f) if math.isfinite(f) else Validation.Fail([f"ParseFloat: {value!r} is not finite"]))
        )
    return _validate_string(value, "ParseFloat").Then(_parse)
...
def ParseDecimal(value: str | None) -> Validation[Decimal, str]:
    def _parse(s: str) -> Validation[Decimal, str]:
        return (
            ...
            .Then(lambda d: Validation.Success(d) if d.is_finite() else Validation.Fail([f"ParseDecimal: {value!r} is not finite"]))
        )
    return _validate_string(value, "ParseDecimal").Then(_parse)
...
def ParseBool(value: Any, truthy: frozenset[str] = _TRUTHY, falsy: frozenset[str] = _FALSY) -> Validation[bool, str]:
    ...
    return _validate_string(str_val, "ParseBool").Then(_lookup)


def ParseDate(value: str | None, fmt: str = "%Y-%m-%d") -> Validation[date, str]:
    return (
        _validate_string(value, "ParseDate")
        .Then(lambda s: Validation.Try(lambda: datetime.strptime(s, fmt).date(), lambda e: [f"ParseDate: cannot parse {value!r} with format {fmt!r}: {e}"]))
    )
...
def ParseDatetime(value: str | None, fmt: str = "%Y-%m-%dT%H:%M:%S") -> Validation[datetime, str]:
    return (
        ...
        .Then(lambda s: Validation.Try(lambda: datetime.strptime(s, fmt), lambda e: [f"ParseDatetime: cannot parse {value!r} with format {fmt!r}: {e}"]))
    )
...
def ParseTime(value: str | None, fmt: str = "%H:%M:%S") -> Validation[time, str]:
    return (
        ...
        .Then(lambda s: Validation.Try(lambda: datetime.strptime(s, fmt).time(), lambda e: [f"ParseTime: cannot parse {value!r} with format {fmt!r}: {e}"]))
    )
...
def ParseUuid(value: str | None) -> Validation[UUID, str]:
    return (
        ...
        .Then(lambda s: Validation.Try(lambda: UUID(s), lambda e: [f"ParseUuid: cannot parse {value!r}: {e}"]))
    )
...
def ParseEnum(value: str | None, enum_type: type[Enum], case_sensitive: bool = False) -> Validation[Enum, str]:
    ...
    return _validate_string(value, "ParseEnum").Then(_lookup)
...
def ParseRegex(value: str | None, pattern: str, group: int | str = 0) -> Validation[str, str]:
    ...
    return _validate_string(value, "ParseRegex").Then(_match)
